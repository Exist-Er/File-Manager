...
public class SearchService {
    ...
    private FileItem toFileItem(Path file, BasicFileAttributes attrs) {
        ...
        if (name.endsWith(".txt") || name.endsWith(".java")
            ...
            return new TextFile(file, size, lastModified);
        } else if (name.endsWith(".jpg") || name.endsWith(".png")
                || name.endsWith(".jpeg")) {
            return new ImageFile(file, size, lastModified);
        } else if (name.endsWith(".mp4") || name.endsWith(".avi")
                || name.endsWith(".mkv")) {
            return new VideoFile(file, size, lastModified);
        } else {
            return new FolderItem(file, size, lastModified, p -> {});
        }
    }
}
